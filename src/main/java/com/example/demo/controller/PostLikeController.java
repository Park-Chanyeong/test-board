...
public class PostLikeController extends BaseController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<LikeDto>> getLikeInfo(
            @PathVariable Long postId,
            @AuthenticationPrincipal UserDetails userDetails) {
        String username = userDetails != null ? userDetails.getUsername() : null;
        return execute(HttpStatus.OK, () ->
                new LikeDto(postLikeService.countByPostId(postId), postLikeService.hasLiked(postId, username)));
    }

    @Operation(summary = "좋아요 토글", description = "좋아요를 누르거나 취소합니다. JWT 인증 필요.")
    @SecurityRequirement(name = "Bearer Authentication")
    @PostMapping
    public ResponseEntity<ApiResponse<LikeDto>> toggleLike(
            @PathVariable Long postId,
            @AuthenticationPrincipal UserDetails userDetails) {
        return execute(HttpStatus.OK, () -> {
